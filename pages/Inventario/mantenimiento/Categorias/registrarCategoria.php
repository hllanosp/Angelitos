  ...
  $(document).ready(function() {
    ...
    de editar la categoria */
    $("#form_Modificar").submit(function(e) {
            e.preventDefault();
            $("#compose-modal-modificar").modal('hide');

            datosEditados = {
                codigo: $("#codCategoria").data('id'),
                nombre: $("#nombreCategoria1").val()
            };
            $.ajax({
                async: true,
                type: "POST",
                ...
                url: "pages/Inventario/Mantenimiento/Categorias/submitModal.php",
    ...
    $(document).on("click",".eliminaCategoria",function () {
        ...
        if (respuesta)
        {
            ...
            $.ajax({
                ...
                url: "pages/Inventario/mantenimiento/Categorias/eliminarCategoria.php",
                timeout: 4000
            });
        }
    });
  ...
  /* Esta funcion es llamada al principio, es la encargada de actualizar la tabla que muestra
   ...
   $(document).on("click",".editarCategoria",function (e){
                ...
                for (var index = 0;index < response.length; index++)
                {
                   options +=   '<div class="form-group">'+
                                ...
                                '<div class="form-group">'+
                                    ...
                                    '<input id="nombreCategoria1" value = "'+response[index].nombreCategoria+'"  placeholder="'+response[index].nombreCategoria+'" class="form-control" required>'+
                                '</div>';
                }
       ...
       /* Evento se levanta cuando queremos insertar una nueva Categoria */
    $("#formInsertar").submit(function (e){
        ...
        $.ajax({
            async: true,
            type: "POST",
            url: "pages/Inventario/mantenimiento/Categorias/insertarCategoria.php",
            data: datos,
            dataType: "html",
            success: function(data){
                $("#notificaciones1").html(data);
